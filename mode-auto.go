...
func (client *Client) parseAutoModeUrl(web3Url *Web3URL) (err error) {
	...
	if methodName == "" {
		return &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Missing method name")}
	}
	...
	if err != nil {
		return &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Unable to parse the query of the URL")}
	}
	...
	for _, queryValue := range parsedQuery {
		if queryValue.Name != "returns" &&
			...
			return &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Unsupported query attribute")}
		}
	}
	...
	if returnTypes != "" {
		if len(returnTypes) < 2 {
			return &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Invalid returns attribute")}
		}
...
func (client *Client) parseArgument(argument string, nsChain int) (abiType abi.Type, typeName string, argValue interface{}, err error) {
	...
	if err != nil {
		err = &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Unable to URI-percent decode: " + argument)}
		return
	}
	...
	if len(ss) >= 2 {
		...
		case "uint", "int":
			...
			if typeSize < 8 || typeSize > 256 || typeSize%8 != 0 {
				err = &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Invalid argument type: " + typeName)}
				return
			}
			...
			if !ok {
				err = &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Argument is not a number: " + argValueStr)}
				return
			}
			if typeWithoutSize == "uint" && n.Cmp(new(big.Int)) == -1 {
				err = &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Number is negative: " + argValueStr)}
				return
			}
			argValue = n
		...
		case "bytes":
			...
			if typeSize == 0 {
				if !has0xPrefix(argValueStr) || !isHex(argValueStr[2:]) {
					err = &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Argument is not a valid hex string: " + argValueStr)}
					return
				}
				...
				if len(argValueStr[2:]) != 2*typeSize {
					err = &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Argument has not the correct length: " + argValueStr)}
					return
				}
		...
		case "bool":
			if argValueStr != "false" && argValueStr != "true" {
				err = &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Argument must be 'true' or 'false'")}
				return
			}
			argValue = argValueStr == "true"

		default:
			err = &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Unknown type: " + typeName)}
			return
		}
...
func parseReturnSignature(text string) (result []abi.ArgumentMarshaling, err error) {
	...
	for _, part := range parts {
		...
		if len(tupleMatches) > 0 {
			...
			if part == "" {
				return result, &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Return attribute: missing type")}
			}
			...
			if err != nil {
				return result, &Web3ProtocolError{HttpCode: http.StatusBadRequest, Err: errors.New("Return attribute: Invalid type: " + part)}
			}
